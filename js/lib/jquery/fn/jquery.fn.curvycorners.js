...
curvyCnrSpec.prototype.setfrom = function (obj) {
	...
	// default to px
	if ('tl' in obj) {
		this.tlR = obj.tl.radius;
	}
	if ('tr' in obj) {
		this.trR = obj.tr.radius;
	}
	if ('bl' in obj) {
		this.blR = obj.bl.radius;
	}
	if ('br' in obj) {
		this.brR = obj.br.radius;
	}
	if ('antiAlias' in obj) {
		this.antiAlias = obj.antiAlias;
	}
};
curvyCnrSpec.prototype.cloneOn = function (box) {
	// not needed by IE
...
function operasheet(sheetnumber) {
	...
	while ((matches = pat.exec(txt)) !== null) {
		var pat2 = new RegExp("(..)border-((top|bottom)-(left|right)-)?radius:\\s*([\\d.]+)(in|em|px|ex|pt)", 
		"g");
...
function curvyCorners() {
	...
	if (arguments[0] instanceof curvyCnrSpec) {
		...
		if (j !== '' && j.charAt(0) !== '.' && 'autoPad' in arguments[0]) {
			j = '.' + j;
		}
		// for compatibility, prefix with dot
	...
	if (settings.selectorText) {
		startIndex = 0;
		var args = settings.selectorText.replace(/\s+$/, '').split(/,\s*/);
		// handle comma-separated selector list
		boxCol = new Array;
		for (i = 0; i < args.length; ++i) {
			if ((j = args[i].lastIndexOf('#')) !== - 1) {
				args[i] = args[i].substr(j);
			}
			// ignore everything on LHS of ID
		...
		if (!skipCorners) {
			if (theBox.className.indexOf('curvyRedraw') !== - 1) {
				...
				curvyCorners.redrawList.push({
					node : theBox, spec : settings, copy : theBox.cloneNode(false) 
				});
			}
...
curvyCorners.redraw = function () {
	...
	for (var i in curvyCorners.redrawList) {
		...
		// in case of added prototype methods
		var o = curvyCorners.redrawList[i];
		if (!o.node.clientWidth) {
			continue;
		}
		// don't resize hidden boxes
		var newchild = o.copy.cloneNode(false);
		for (var contents = o.node.firstChild; contents !== null; contents = contents.nextSibling){
			if (contents.className.indexOf('autoPadDiv') !== - 1) {
				break;
			}
			if (!contents) {
			...
			while (contents.firstChild) {
				newchild.appendChild(contents.removeChild(contents.firstChild));
			}
...
function curvyObject() {
	...
	// browser-independent IE-emulation (NB includes padding)
	if (('canHaveChildren' in this.box && !this.box.canHaveChildren) || this.box.tagName === 'TABLE'){
		throw new Error(this.errmsg("You cannot apply corners to " + this.box.tagName + " elements.", 
		"Error"));
	}
	if (!boxWidth && curvyBrowser.isIE) {
		this.box.style.zoom = 1;
		// can force IE to calculate width
	...
	if (!boxWidth) {
		...
		for (boxDisp = this.box; ; boxDisp = boxDisp.parentNode) {
			if (!boxDisp || boxDisp.tagName === 'BODY') {
				// we've hit the buffers
				this.applyCorners = function () {};
				// make the error benign
				curvyCorners.alert(this.errmsg("zero-width box with no accountable parent", "warning"));
				return;
			}
			if (curvyBrowser.get_style(boxDisp, 'display') === 'none') {
				break;
			}
		}
		...
		// display in order to get browser to calculate clientWidth
		boxWidth = this.box.clientWidth;
	}
	...
	var filter = curvyBrowser.ieVer > 7 ? curvyBrowser.get_style(this.box, 'filter') : null;
	// IE8 bug fix
	...
	var styleToNPx = function (val) {
		...
		if (typeof val !== 'string') {
			throw new Error('unexpected styleToNPx type ' + typeof val);
		}
		var matches = /^[-\d.]([a-z]+)$/.exec(val);
		if (matches && matches[1] != 'px') {
			throw new Error('Unexpected unit ' + matches[1]);
		}
	...
	var clientWidth = boxWidth;
	// save it as it gets trampled on later
	if (curvyBrowser.isOp) {
		...
		if (backgroundPosX) {
			...
			backgroundPosX = (t  / backgroundPosX * 100) + '%';
		...
		if (backgroundPosY) {
			...
			backgroundPosY = (t  / backgroundPosY * 100) + '%';
			// convert to percentage
		}
	}
	...
	$(this.box).css({
		...
		width:(clientWidth + this.borderWidthL + this.borderWidthR) + 'px',
		height:(clientHeight + this.borderWidth + this.borderWidthB) + 'px'
	});
	// Ok we add an inner div to actually put things into this will allow us to keep the height
	var newMainContainer = document.createElement("div");
	newMainContainer.style.position = "absolute";
	if (filter) {
		newMainContainer.style.filter = filter;
	}
	// IE8 bug fix
	if (curvyBrowser.quirksMode) {
	...
	if (boxWidth === "" || boxWidth === "auto" || boxWidth.indexOf("%") !== - 1) {
		throw this.newError('Shell width is ' + boxWidth);
	}
	...
	*/
	this.applyCorners = function () {
		...
		*/
		...
		if (this.backgroundObject) {
			var bgOffset = function (style, imglen, boxlen) {
				...
				if (style === 'right' || style === 'bottom') {
					return boxlen - imglen;
				}
				if (style === 'center') {
					return (boxlen - imglen)  / 2;
				}
				if (style.indexOf('%') > 0) {
					return (boxlen - imglen) * 100  / parseInt(style);
				}
		...
		*/
		...
		var corners = this.spec.cornerNames();
		...
		*/
		for (var i in corners){
			if (!isNaN(i)) {
				...
				$(newCorner).css({
					height:this.spec.get(cc + 'Ru'),
					width:this.spec.get(cc + 'Ru'),
					position:"absolute",
					fontSize:"1px",
					overflow:"hidden"
				});
				...
				for (intx = 0; intx < specRadius; ++intx) {
					...
					var y1 = (intx + 1 >= borderRadius) ? - 1 : Math.floor(Math.sqrt(Math.pow(borderRadius, 
					2) - Math.pow(intx + 1, 2))) - 1;
					// Calculate y2 and y3 only if there is a border defined
					if (borderRadius != specRadius) {
						var y2 = (intx >= borderRadius) ? - 1 : Math.ceil(Math.sqrt(Math.pow(borderRadius, 
						2) - Math.pow(intx, 2)));
						var y3 = (intx + 1 >= specRadius) ? - 1 : Math.floor(Math.sqrt(Math.pow(specRadius, 
						2) - Math.pow((intx + 1), 2))) - 1;
					}
					// Calculate y4
					var y4 = (intx >= specRadius) ? - 1 : Math.ceil(Math.sqrt(Math.pow(specRadius, 2) - Math.pow(intx, 
					2)));
					// Draw bar on inside of the border with foreground colour
					if (y1 > - 1) {
						this.drawPixel(intx, 0, this.boxColour, trans, (y1 + 1), newCorner, true, specRadius);
					}
					// Draw border/foreground antialiased pixels and border only if there is a border defined
					if (borderRadius != specRadius) {
						// Cycle the y-axis
						//alert(this.spec.antiAlias)
						if (this.spec.antiAlias) {
							for (inty = y1 + 1; inty < y2; ++inty) {
								// For each of the pixels that need anti aliasing between the foreground and border colour draw single pixel divs
								//alert(this.boxColour)
								if (this.backgroundImage !== "") {
									var borderFract = curvyObject.pixelFraction(intx, inty, borderRadius) * 100;
									this.drawPixel(intx, inty, bcolor, trans, 1, newCorner, borderFract >= 30, 
									specRadius);
								}
								else if (this.boxColour !== 'transparent') {
									var pixelcolour = curvyObject.BlendColour(this.boxColour, bcolor, 
									curvyObject.pixelFraction(intx, inty, borderRadius));
									this.drawPixel(intx, inty, pixelcolour, trans, 1, newCorner, false, 
									specRadius);
								}
								else {
									this.drawPixel(intx, inty, bcolor, trans >> 1, 1, newCorner, false, 
									specRadius);
								}
							}
							...
							if (y3 >= y2) {
								...
								this.drawPixel(intx, y2, bcolor, trans, (y3 - y2 + 1), newCorner, false, 
								0);
							}
							outsideColour = bcolor;
							// Set the colour for the outside AA curve
							inty = y3;
							// start_pos - 1 for y-axis AA pixels
						}
						else {
							// no antiAlias
							if (y3 > y1) {
								// NB condition was >=, changed to avoid zero-height divs
								this.drawPixel(intx, (y1 + 1), bcolor, trans, (y3 - y1), newCorner, false, 
								0);
							}
						}
					}
					else {
						outsideColour = this.boxColour;
					...
					if (this.spec.antiAlias && this.boxColour !== 'transparent') {
						...
						while (++inty < y4) {
							...
							this.drawPixel(intx, inty, outsideColour, (curvyObject.pixelFraction(intx, 
							inty , specRadius) * trans), 1, newCorner, borderWidthTB <= 0, specRadius);
						}
					}
				}
				...
				*/
				...
				for (t = 0, k = newCorner.childNodes.length; t < k; ++t) {
					...
					// Reposition pixels
					if (cc == "tl" || cc == "bl") {
						pixelBar.style.left = (specRadius - pixelBarLeft - 1) + "px";
						// Left
					}
					if (cc == "tr" || cc == "tl") {
						pixelBar.style.top = (specRadius - pixelBarHeight - pixelBarTop) + "px";
						// Top
					}
			...
			*/
			...
			var radiusDiff = {
				t : this.spec.radiusdiff('t'), b : this.spec.radiusdiff('b') 
			};
			for (var z in radiusDiff) {
				if (typeof z === 'function'){
					continue;
				}
				// for prototype, mootools frameworks
				if (!this.spec.get(z + 'R')) {
					continue;
				}
				// no need if no corners
				if (radiusDiff[z]) {
					...
					switch (smallerCornerType) {
						case "tl":
							newFiller.style.bottom = newFiller.style.left = "0";
							...
							break;
						case "tr":
							newFiller.style.bottom = newFiller.style.right = "0";
							...
							break;
						case "bl":
							newFiller.style.top = newFiller.style.left = "0";
							newFiller.style.borderLeft = this.borderStringL;
							// Set background image in original position
							...
							break;
						case "br":
							newFiller.style.top = newFiller.style.right = "0";
							newFiller.style.borderRight = this.borderStringR;
							// Set background image in original position.
				...
				if (filter) {
					newFillerBar.style.filter = filter;
				}
				// IE8 bug fix
				$(newFillerBar).css({
					position:"relative",
				...
				switch (z) {
					case "t":
						...
						if (this.topContainer) {
							...
							if (this.backgroundImage) {
								var x_offset = this.spec.tlR ? (this.borderWidthL + this.backgroundPosX - this.spec.tlR) + "px " : this.backgroundPosX + "px ";
					...
					case "b":
						if (this.bottomContainer) {
							...
							if (this.backgroundImage) {
								var x_offset = this.spec.blR ? (this.backgroundPosX + this.borderWidthL - this.spec.blR) + "px " : this.backgroundPosX + "px ";
			...
			$(this.contentContainer).css({
				position:"absolute",
				// border:"1px dotted #000", // DEBUG, comment for production
				left:this.borderWidthL + "px",
				// Get padding amounts
				// Apply top padding
			...
			// important otherwise layout goes wild
...
curvyObject.prototype.backgroundCheck = function (style) {
	if (style === 'top' || style === 'left' || parseInt(style) === 0) {
		return 0;
	}
...
curvyObject.dispatch = function (obj) {
	if ('dispatch' in obj) {
		...
		throw obj.newError('No dispatch function');
	}
};
// append a pixel DIV to newCorner
curvyObject.prototype.drawPixel = function (intx, inty, colour, transAmount, height, newCorner, image, 
cornerRadius) {
	...
	// Set opacity if the transparency is anything other than 100//修改为最大99.999
	
	curvyObject.setOpacity(pixel, transAmount);
...
curvyObject.BlendColour = function (Col1, Col2, Col1Fraction) {
	...
	endRed = Math.min(endRed, 255);
	endRed = Math.max(endRed, 0);
	var endGreen = Math.round((green1 * Col1Fraction) + (green2 * (1 - Col1Fraction)));
	endGreen = Math.min(endGreen, 255);
	endGreen = Math.max(endGreen, 0);
	var endBlue = Math.round((blue1 * Col1Fraction) + (blue2 * (1 - Col1Fraction)));
	endBlue = Math.min(endBlue, 255);
	endBlue = Math.max(endBlue, 0);
...
curvyObject.pixelFraction = function (x, y, r) {
	...
	*/
	switch (whatsides) {
		case "LeftRight":
			fraction = Math.min(yvalues[0], yvalues[1]) + ((Math.max(yvalues[0], yvalues[1]) - Math.min(yvalues[0], 
			yvalues[1]))  / 2);
			break;
		case "TopRight":
			...
			break;
		case "TopBottom":
			fraction = Math.min(xvalues[0], xvalues[1]) + ((Math.max(xvalues[0], xvalues[1]) - Math.min(xvalues[0], 
			xvalues[1]))  / 2);
			break;
		case "LeftBottom":
...
*/
curvyObject.setOpacity = function (obj, opacity) {//这里有问题
	//alert(opacity);
...
curvyObject.getComputedColour = function (colour) {
	...
	if (window.getComputedStyle) {
		...
		if (rtn.substr(0, 3) === "rgb") {
			rtn = curvyObject.rgb2Hex(rtn);
		}
		return rtn;
	}
	else {
...
curvyCorners.getElementsByClass = function (searchClass, node) {
	...
	searchClass = searchClass.split('.');
	// see if there's a tag in there
	var tag = '*';
	// prepare for no tag
...
if (curvyBrowser.supportsCorners) {
	...
	curvyCorners.scanStyles = function () {
		function units(num) {
			...
			// '0' becomes '0px' for simplicity's sake
			...
			function procIEStyles(rule) {
				...
				if (allR || tL || tR || bR || bL) {
					...
					if (allR) {
						...
						if (tR) {
							settings.setcorner('t', 'r', parseInt(tR), units(tR));
						}
						if (tL) {
							settings.setcorner('t', 'l', parseInt(tL), units(tL));
						}
						if (bL) {
							settings.setcorner('b', 'l', parseInt(bL), units(bL));
						}
						if (bR) {
							settings.setcorner('b', 'r', parseInt(bR), units(bR));
						}
					}
