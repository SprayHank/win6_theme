...
function addRoundedIECorners() {
	...
	for(i=0;i<theimages.length;i++) {	
		...
		path += " x e"; data = '<v:shape'+
								' strokeweight="'+iborder+'px"'+
								' stroked="'+(iborder>0?"t":"f")+'"'+
								' strokecolor="#'+color+'"'+
								' filled="t"'+
								' fillcolor="#ffffff"'+
								' coordorigin="0,0"'+
								' coordsize="' + image.width + ',' + image.height + '"'+
								' path="' + path + '"'+
								' style="zoom:1;margin:-1px 0 0 -1px;padding: 0;display:' + display + ';width:' + image.width + 'px;height:' + image.height + 'px;">'+
								'<v:fill src="' + image.src + '" type="frame" /></v:shape>';
